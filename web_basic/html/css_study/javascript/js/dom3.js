...
addbutton[0].onclick = () => {
    ...
    <li><input type="checkbox" class="ok-check"><span class="todo-content">${todotext[0].value}</span><button class="ok-button">확인</button></li>
    `
    todotext[0].value = "";
...
for(let i = 0 ; i< okbutton.length;i++){
    okbutton[i].onclick = () => {
        if(todocontent[i].style.color == "" || todocontent[i].style.color == "black" ){
